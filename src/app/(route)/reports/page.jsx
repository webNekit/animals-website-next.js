            ...
            <section className="w-full py-12">
                <div className="container mx-auto">
                    <div className="w-full grid grid-grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5">
                        {data.map((item, index) => {
                            <div key={index} className="w-full rounded-lg bg-blue-50 p-5">
                                ...
                                <a href={process.env.NEXT_PUBLIC_STRAPI_API_URL + (item.attributes?.reportFile?.data.url ?? '')} download className='mt-5 inline-flex items-center gap-4 h-10 pl-4 border bg-white rounded-full group'>
