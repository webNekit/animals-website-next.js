                ...
                return (
                    <div key={index} className="w-full bg-white border p-3 rounded-lg">
                        <div className="w-full h-[320px] overflow-hidden rounded-lg group relative">
                            ...
                            <Image src={process.env.NEXT_PUBLIC_STRAPI_API_URL + (item.attributes?.image?.data.attributes?.url ?? '')} className='group-hover:opacity-0 absolute z-20 top-0 right-0 bottom-0 left-0 w-full h-full object-cover object-center transition-all duration-500' alt='Изображение собаки' width={130} height={240} />
                            <Image src={process.env.NEXT_PUBLIC_STRAPI_API_URL + (item.attributes?.otherImages?.data[0].attributes?.url ?? '')} className='absolute z-10 top-0 right-0 bottom-0 left-0 w-full h-full object-cover' alt='Изображение собаки' width={130} height={240} />
                        </div>
